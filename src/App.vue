<template>
    <DlThemeProvider :is-dark="isDark">
        ...
        <DlTypography variant="h1">
            ...
            <DlButton :label="buttonLabel" :disabled="buttonStatus" @click="onClick" />
            <DlProgressBar label="Progress bar"
                v-bind="{ width: '200px',  value: 0.5, showValue: true, showPercentage: true }" />
            </div>
        </DlTypography>
    </DlThemeProvider>
    <div class="container">
        <iframe class="frame-container" ref="contentIframe" :src="iframeSource"></iframe>
    </div>
</template>

<script setup lang="ts">
import { DlThemeProvider, DlTypography, DlButton,DlProgressBar } from '@dataloop-ai/components'
import { DlEvent } from '@dataloop-ai/jssdk'
import { onMounted } from 'vue'
import { ref } from 'vue-demi'

let iframeSource = ref('src/loading.html');
// let iframeSource = ref('http://localhost:3003/insights/main');
let isDark = ref(true)
let lastUpdated = ref('Never');
let buttonStatus = ref(false);
let buttonLabel = ref('Run');
let dataset_id = null
let project_id = null
